...
import { PromoteMemberUseCase } from './promote-member.use-case';
...
describe('PromoteMemberUseCase', () => {
  ...
  beforeEach(() => {
    ...
    useCase = new PromoteMemberUseCase(groupRepo);
  });
  ...
  it('throws BadRequestException when target is already a MODERATOR', async () => {
    groupRepo.findMember
      .mockResolvedValueOnce(buildMember('caller-1', MemberRole.OWNER))
  ...
  it('promotes an ACTIVE MEMBER to MODERATOR when caller is OWNER', async () => {
    groupRepo.findMember
      .mockResolvedValueOnce(buildMember('caller-1', MemberRole.OWNER))
